...
half3 CustomCalculateLightingColor(LightingData lightingData, half3 albedo)
{
    ...
    half r = lightingColor.r, g = lightingColor.g, b = lightingColor.b;
    float lightness = (max(r, max(g, b)) + min(r, min(g, b)))/2;
...
half ColorIntensity(half3 v){
    return (max(max(v.r, v.g), v.b) + min(min(v.r, v.g), v.b))/2;
}
...
half4 CustomLighting(InputData inputData, SurfaceData surfaceData)
{
   ...
   lightIntensity += ColorIntensity(inputData.bakedGI);

#ifdef _LIGHT_LAYERS
    if (IsMatchingLightLayer(mainLight.layerMask, meshRenderingLayers))
#endif
    {
        lightIntensity += ColorIntensity(CustomCalculateBlinnPhong(mainLight, inputData, surfaceData));
    }
...
#endif
        {
            // lightIntensity += ColorIntensity(CustomCalculateBlinnPhong(light, inputData, surfaceData));
        }
    }
    #endif
...
#endif
{
    ...
    half4 color = CustomLighting(inputData, surfaceData);
